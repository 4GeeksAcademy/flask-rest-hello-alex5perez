...
def get_single_person(people_id):
    ...
    if person is None:
        return jsonify({'msg': f'El personaje con id {people_id} no existe'})
...
def get_single_planet(planet_id):
    ...
    return jsonify({'msg': 'ok', 'data': planet_serialized})
...
def get_favorites_by_user(user_id):
    ...
    return jsonify({'favorite_planets': favorite_planets_serialized, 'user': favorite_planets[0].users.serialize()})
